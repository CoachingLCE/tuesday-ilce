...
function traducirError(err) {
  ...
  if (/storage quota/i.test(msg)) {
    return new Error('La carpeta de Drive configurada (GOOGLE_DRIVE_FOLDER_ID) no es una carpeta dentro de una Unidad compartida. Las cuentas de servicio no tienen almacenamiento propio, así que la carpeta tiene que estar dentro de una Unidad compartida (Shared Drive) de Google Workspace, compartida con la cuenta de servicio. Ver SETUP.md.');
  }
...
export async function subirArchivo({ buffer, nombre, mimeType }) {
  ...
  try {
    res = await drive.files.create({
      ...
      // (Shared Drive) — que es donde TIENE que estar, porque la cuenta de servicio no
      // tiene almacenamiento propio (ver SETUP.md).
