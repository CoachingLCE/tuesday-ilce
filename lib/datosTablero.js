...
export async function leerActividad(itemId) {
  ...
  return filas.filter((f) => f.Id && (!itemId || f.ItemId === itemId)).map((f) => ({
    ...
    // A quién le corresponde como notificación personal (se lo mencionó con @, o se lo
    // asignó como responsable) — columna "Para" opcional (array de emails en JSON); si el
    // Sheet todavía no la tiene, o la fila es vieja, queda vacío y solo cuenta como
    // actividad general del tablero, no como notificación para nadie en particular.
    para: parseJson(f.Para, [])
